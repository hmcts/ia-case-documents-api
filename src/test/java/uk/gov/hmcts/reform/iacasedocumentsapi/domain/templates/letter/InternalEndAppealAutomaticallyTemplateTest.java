...
class InternalEndAppealAutomaticallyTemplateTest {
    @Mock
    ...
    private String appealSubmissionDate = "2023-07-01";
    private String remissionRejectedDate = "14 Jul 2023";
    private final String templateName = "INTERNAL_END_APPEAL_AUTOMATICALLY_NOTICE_TEMPLATE.docx";
    private final String logo = "[userImage:hmcts.png]";
    private InternalEndAppealAutomaticallyTemplate internalEndAppealAutomaticallyTemplate;
    private Map<String, Object> fieldValuesMap;

    @BeforeEach
    public void setUp() {
        internalEndAppealAutomaticallyTemplate =
                new InternalEndAppealAutomaticallyTemplate(templateName, customerServicesProvider);
    }

    @Test
    void should_return_template_name() {
        assertEquals(templateName, internalEndAppealAutomaticallyTemplate.getName());
    }
    ...
    void should_populate_template() {
        dataSetup();
        fieldValuesMap = internalEndAppealAutomaticallyTemplate.mapFieldValues(caseDetails);
        ...
        assertEquals(LocalDate.parse(appealSubmissionDate).plusDays(14).format(DateTimeFormatter.ofPattern("d MMM yyyy")), fieldValuesMap.get("deadLineDate"));
    }

    @Test
    void should_return_remission_rejected_date() {
        dataSetup();
        when(asylumCase.read(REMISSION_TYPE, RemissionType.class)).thenReturn(Optional.of(RemissionType.HO_WAIVER_REMISSION));
        when(asylumCase.read(REMISSION_REJECTED_DATE_PLUS_14DAYS, String.class)).thenReturn(Optional.of(remissionRejectedDate));
        fieldValuesMap = internalEndAppealAutomaticallyTemplate.mapFieldValues(caseDetails);
        assertEquals(remissionRejectedDate, fieldValuesMap.get("deadLineDate"));
    }
    ...
    void should_throw_when_appeal_submission_date_not_present() {
        ...
        assertThatThrownBy(() -> internalEndAppealAutomaticallyTemplate.mapFieldValues(caseDetails))
    ...
    @Test
    void should_throw_when_remission_date_not_present() {
        dataSetup();
        when(asylumCase.read(REMISSION_TYPE, RemissionType.class)).thenReturn(Optional.of(RemissionType.HO_WAIVER_REMISSION));
        when(asylumCase.read(REMISSION_REJECTED_DATE_PLUS_14DAYS, String.class)).thenReturn(Optional.empty());

        assertThatThrownBy(() -> internalEndAppealAutomaticallyTemplate.mapFieldValues(caseDetails))
                .isExactlyInstanceOf(IllegalStateException.class)
                .hasMessage("Remission rejected date is missing");
