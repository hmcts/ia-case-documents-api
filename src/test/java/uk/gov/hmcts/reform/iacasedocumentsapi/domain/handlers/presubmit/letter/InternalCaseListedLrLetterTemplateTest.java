...
import static uk.gov.hmcts.reform.iacasedocumentsapi.domain.entities.AsylumCaseDefinition.LEGAL_REP_FAMILY_NAME_PAPER_J;
...
class InternalCaseListedLrLetterTemplateTest {
    ...
    private InternalCaseListedLrLetterTemplate internalCaseListedLrLetterTemplate;
    private Map<String, Object> fieldValuesMap;

    @BeforeEach
    public void setUp() {
        internalCaseListedLrLetterTemplate =
            new InternalCaseListedLrLetterTemplate(templateName, customerServicesProvider, stringProvider);
    }

    @Test
    void should_return_template_name() {
        assertEquals(templateName, internalCaseListedLrLetterTemplate.getName());
    }

    @Test
    void should_populate_template_correctly_legal_rep_in_uk() {
        dataSetup(true);
    ...
    @Test
    void should_populate_template_correctly_legal_rep_ooc() {
        dataSetup(false);
    ...
    void dataSetup(boolean legalRepInUk) {
        ...
        if (legalRepInUk) {
            ...
            when(asylumCase.read(OOC_ADDRESS_LINE_1, String.class)).thenReturn(Optional.of(addressLine1));
            when(asylumCase.read(OOC_ADDRESS_LINE_2, String.class)).thenReturn(Optional.of(addressLine2));
            when(asylumCase.read(OOC_ADDRESS_LINE_3, String.class)).thenReturn(Optional.of(addressLine3));
            when(asylumCase.read(OOC_ADDRESS_LINE_4, String.class)).thenReturn(Optional.of(postTown));
