...
public class LaunchDarklyFeatureTogglerTest {
    ...
    void should_isFlagKnown_propagate_the_value_returned_by_ldClient(boolean val) {
        when(ldClient.isFlagKnown("flag_name")).thenReturn(val);

        assertEquals(val, launchDarklyFeatureToggler.isFlagKnown("flag_name"));
    }
}
