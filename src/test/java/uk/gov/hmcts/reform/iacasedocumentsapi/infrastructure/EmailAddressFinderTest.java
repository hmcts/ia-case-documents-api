...
public class EmailAddressFinderTest {
    ...
    void setup() {
        ...
        when(adminEmailAddresses.get(HearingCentre.HARMONDSWORTH)).thenReturn("harmondsworthedets@example.com");
        when(adminEmailAddresses.get(HearingCentre.YARLS_WOOD)).thenReturn("yarlswoodedets@example.com");
    ...
    public void should_return_given_list_case_email_address_from_lookup_map() {
        assertEquals(
            listCaseHearingCenterEmailAddress,
    ...
    public void should_return_given_list_case_ftpa_email_address_from_lookup_map() {
        assertEquals(
            listCaseHearingCenterEmailAddress,
            emailAddressFinder.getListCaseFtpaHomeOfficeEmailAddress(asylumCase)
        );
        when(asylumCase.read(
            LIST_CASE_HEARING_CENTRE,
            HearingCentre.class
    ...
    @ParameterizedTest
    @CsvSource({
        "BIRMINGHAM,birminghamedets@example.com",
        "NOTTINGHAM,birminghamedets@example.com",
        "COVENTRY,birminghamedets@example.com",
        "BRADFORD,bradfordedets@example.com",
    ...
    public void should_return_correct_admin_email_address_from_lookup_map(String hearingCentre, String mappedEmail) {
        // Given
        // The addresses defined in the adminEmailAddresses

        // When
        when(asylumCase.read(AsylumCaseDefinition.HEARING_CENTRE, HearingCentre.class)).thenReturn(Optional.of(
            HearingCentre.valueOf(hearingCentre)));
    ...
    public void should_throw_exception_on_bail_hearing_centre_when_is_empty() {
        when(bailCase.read(
            BailCaseFieldDefinition.HEARING_CENTRE,
            BailHearingCentre.class
    ...
    public void should_return_correct_bail_hearing_Centre_email_address_from_lookup_map() {
        ...
        when(bailHearingCentreEmailAddresses.get(BailHearingCentre.HATTON_CROSS)).thenReturn(
            "hc-hatton-cross@example.com");
