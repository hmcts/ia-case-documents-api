...
class InternalCmrReListingLegalRepLetterBundlerTest {
    ...
    private InternalCmrReListingLegalRepLetterBundler handler;

    @BeforeEach
    void setUp() {

        handler = new InternalCmrReListingLegalRepLetterBundler(
                "pdf",
                "internal-letter",
                true,
                fileNameQualifier,
    ...
    void should_bundle_non_detained_case() {
        ...
        try (MockedStatic<uk.gov.hmcts.reform.iacasedocumentsapi.domain.utils.AsylumCaseUtils> mocked =
            ...
            mocked.when(() ->
                            getMaybeLetterNotificationDocuments(asylumCase, DocumentTag.INTERNAL_CMR_RE_LISTING_LETTER))
                    .thenReturn(appellantDocs);

            mocked.when(() ->
                            getMaybeLetterNotificationDocuments(asylumCase, DocumentTag.INTERNAL_CMR_RE_LISTING_LR_LETTER))
                    .thenReturn(lrDocs);
