...
class InternalDetainedAppealSubmissionOutOfTimeWithRemissionIrcPrisonTemplateTest {
    ...
    private InternalDetainedAppealSubmissionOutOfTimeWithRemissionIrcPrisonTemplate template;

    @BeforeEach
    void setUp() {
        template = new InternalDetainedAppealSubmissionOutOfTimeWithRemissionIrcPrisonTemplate(
                templateName,
    ...
    @Test
    void should_handle_blank_fee_amount() {
        when(caseDetails.getCaseData()).thenReturn(asylumCase);
        when(customerServicesProvider.getInternalCustomerServicesEmail(asylumCase)).thenReturn(internalCustomerServicesEmail);
        when(customerServicesProvider.getInternalCustomerServicesTelephone(asylumCase)).thenReturn(internalCustomerServicesTelephone);
        when(systemDateProvider.dueDate(daysAfterSubmitAppeal)).thenReturn(formatDateForNotificationAttachmentDocument(dueDate));
    ...
    @Test
    void should_handle_zero_fee_amount() {
        when(caseDetails.getCaseData()).thenReturn(asylumCase);
        when(customerServicesProvider.getInternalCustomerServicesEmail(asylumCase)).thenReturn(internalCustomerServicesEmail);
        when(customerServicesProvider.getInternalCustomerServicesTelephone(asylumCase)).thenReturn(internalCustomerServicesTelephone);
        when(systemDateProvider.dueDate(daysAfterSubmitAppeal)).thenReturn(formatDateForNotificationAttachmentDocument(dueDate));
        ...
        assertEquals(11, templateFieldValues.size());
        assertEquals("0.00", templateFieldValues.get("feeAmount"));
    }
