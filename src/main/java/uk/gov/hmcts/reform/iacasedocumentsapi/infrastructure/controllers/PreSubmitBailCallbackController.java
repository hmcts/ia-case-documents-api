...
@RestController
public class PreSubmitBailCallbackController {

    private static final org.slf4j.Logger LOG = getLogger(PreSubmitBailCallbackController.class);

    private final PreSubmitCallbackDispatcher<BailCase> callbackDispatcher;

    public PreSubmitBailCallbackController(
        PreSubmitCallbackDispatcher<BailCase> callbackDispatcher
    ) {
    ...
    @ApiOperation(
        value = "Handles 'AboutToStartEvent' callbacks from CCD or delegated calls from IA BAIL Case API",
