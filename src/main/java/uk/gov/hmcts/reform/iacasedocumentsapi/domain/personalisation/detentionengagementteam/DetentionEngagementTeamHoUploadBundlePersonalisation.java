...
import static uk.gov.hmcts.reform.iacasedocumentsapi.domain.entities.DocumentTag.INTERNAL_DETAINED_APPEAL_HO_UPLOAD_BUNDLE_APPELLANT_LETTER;
...
@Slf4j
@Service
public class DetentionEngagementTeamHoUploadBundlePersonalisation implements EmailWithLinkNotificationPersonalisation {
    ...
    public DetentionEngagementTeamHoUploadBundlePersonalisation(
            @Value("${govnotify.template.requestRespondentEvidenceDirection.detentionEngagementTeam.appellant.email}") String detentionEngagementTeamTemplateId,
    ...
    public String getReferenceId(Long caseId) {
        return caseId + "_INTERNAL_DETAINED_APPEAL_HO_UPLOAD_BUNDLE_APPELLANT_LETTER";
    }
    ...
    private JSONObject getRemissionGrantedInTimeLetterJsonObject(AsylumCase asylumCase) {
        try {
            return documentDownloadClient.getJsonObjectFromDocument(getLetterForNotification(asylumCase, INTERNAL_DETAINED_APPEAL_HO_UPLOAD_BUNDLE_APPELLANT_LETTER));
