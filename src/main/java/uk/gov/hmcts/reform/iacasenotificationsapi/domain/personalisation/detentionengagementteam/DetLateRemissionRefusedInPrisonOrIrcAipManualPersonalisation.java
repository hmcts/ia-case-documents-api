...
import static uk.gov.hmcts.reform.iacasenotificationsapi.domain.entities.DocumentTag.INTERNAL_DETAINED_LATE_REMISSION_REFUSED_TEMPLATE_LETTER;
import static uk.gov.hmcts.reform.iacasenotificationsapi.domain.utils.AsylumCaseUtils.getLetterForNotification;
import static uk.gov.hmcts.reform.iacasenotificationsapi.domain.utils.AsylumCaseUtils.isAppellantInDetention;

@Slf4j
@Service
public class DetLateRemissionRefusedInPrisonOrIrcAipManualPersonalisation implements EmailWithLinkNotificationPersonalisation {
    ...
    public DetLateRemissionRefusedInPrisonOrIrcAipManualPersonalisation(
    ...
    public String getReferenceId(Long caseId) {
        return caseId + "_INTERNAL_DETAINED_LATE_REMISSION_REFUSED_IN_PRISON_OR_IRC";
    }
    ...
    private JSONObject getAppealCanProceedLetterJsonObject(AsylumCase asylumCase) {
        try {
            return documentDownloadClient.getJsonObjectFromDocument(getLetterForNotification(asylumCase,
                    INTERNAL_DETAINED_LATE_REMISSION_REFUSED_TEMPLATE_LETTER));
        } catch (IOException | NotificationClientException e) {
            String msg = "Failed to get 'INTERNAL_DETAINED_LATE_REMISSION_REFUSED_TEMPLATE_LETTER' Letter in compatible format";
            log.error(msg, e);
